...
if __name__ == '__main__':
    ...
    if args.dataset == "cifar100":
        ...
        train_loader, test_loader = utils.get_cifar100(args.batch_size)
    else:
        print("loading cifar-10 dataset...")
        train_loader, test_loader = utils.get_cifar10(args.batch_size)
    ...
    # Open logger. Default is empty. Change in arguments.
    do_log = False
    if args.resfile != "":
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0 or epoch == args.epochs -1:
            ...
            print()
            logline = "TEST [%s: e-%d a-%s d-%d-%.3f n-%s l-%.3f] : %.4f" %(args.dataset, epoch, args.approach, dims, args.radius, args.network, train_scheduler.get_last_lr()[0], acc)
            torch.save(model.state_dict(), 'ckpts/'+args.dataset+'_'+args.network+'_'+args.approach+'.pt')
            if do_log:
